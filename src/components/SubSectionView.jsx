...
const SubSectionView = ({ sub, onClose }) => {
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-50 bg-[#030014]/95 backdrop-blur-3xl overflow-y-auto w-full h-full flex flex-col"
    >
        <div className="w-full max-w-7xl mx-auto px-6 py-12 flex flex-col h-full">
            {/* Header & Back Button */}
            ...
            <motion.div layoutId={`card-container-${sub.id}`} className="mb-12">
                <motion.h2 layoutId={`card-title-${sub.id}`} className="text-4xl md:text-6xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400 tracking-tight">{sub.title}</motion.h2>
                <p className="text-gray-300 text-lg md:text-xl leading-relaxed max-w-4xl font-light">
                    {sub.content.text}
                </p>
            </motion.div>
            ...
            {sub.content.gallery && sub.content.gallery.length > 0 && (
               <div className="mb-16 mt-8">
                 <h4 className="text-xl font-semibold mb-6 text-blue-300 border-b border-white/10 pb-2 inline-block">Visual Evidence</h4>
                 ...
                 <div className="relative w-full rounded-3xl overflow-hidden glass-panel p-2">
                     ...
                     >
                        ...
                        {sub.content.gallery.map((imgUrl, i) => (
                           <div key={i} className="min-w-[85vw] md:min-w-[800px] h-[300px] md:h-[500px] snap-center shrink-0 rounded-2xl overflow-hidden shadow-2xl border border-white/10 relative group">
            ...
            {/* Documents Section */}
             {sub.content.docs && sub.content.docs.length > 0 && (
                 <div className="mb-16">
                     <h4 className="text-xl font-semibold mb-6 text-blue-300 border-b border-white/10 pb-2 inline-block">Linked Documents</h4>
